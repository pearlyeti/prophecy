...
| Object storage | Cloudflare R2 | Card art, audio, replays, cosmetic assets. Card art source files: JPEG / PNG / WebP, max 20 MB. Server converts to WebP at 1024 × 1024 px / quality 85 before storing. |
...
- **Board layout — mobile-first, top to bottom:**
  1. **Avatar bar** — player avatar + resources + deck count (left) | battlefield card + controller indicator (center) | opponent avatar + hand count + deck count (right). The battlefield card tilts toward whichever player controls it. Tapping the battlefield card claims it (when available). The action panel (⚡) lives here until direct-tap actions cover all cases.
  2. **Opponent zone** — opponent character and support cards, with each card's dice directly below it. Front row (most cards) sits at the bottom of the opponent zone, closest to the center. Multiple rows stack upward.
  3. **Player zone** — player character and support cards, with each card's dice directly above it. Front row sits at the top of the player zone, closest to the center. Multiple rows stack downward.
  4. **Hand strip** — always visible compact row: small art swatch + card name + cost badge. Tapping a card expands it in place to show ability text. Eligible-to-play cards have a green border.
  5. **Action bar** — Undo (left, hidden when no action is pending) | Commit (right, label changes by turn state — see Turn interaction model below). The left slot also shows "Discard to reroll" when idle and that action is legal.
  - Cards use square tiles for now (CCG ratio is a future upgrade). Exhausted cards rotate slightly. Upgrade cards appear as circular badge overlays on their attached card; tapping a badge opens the upgrade detail. Dice always stay upright.
  - **Column width is dynamic.** Each character (and support) gets a column wide enough to fit up to 3 dice side by side: `max(diceCount, 3) × 44 pt + gaps`. Characters pack left-to-right into rows until adding the next column would overflow the viewport; a new row starts. More characters go in front rows (closest to center).
- **Turn interaction model.** On your turn, eligible objects highlight green. Tapping one enters an action flow and clears all other highlights. Only one flow is active at a time. Key rules:
  - **Idle state:** Commit = "Pass" (confirmation dialog required). Undo is hidden. All eligible objects are highlighted: ready characters (can activate), resolvable dice (non-blank, non-modifier-alone), affordable hand cards, battlefield card (if claimable), card ability badges (if available).
  - **Activate a character:** tap the green character → it tilts (client preview), Commit = "Roll Dice", Undo or re-tap reverts. No confirmation on Roll Dice.
  - **Resolve dice:** tap a non-modifier die first (required) → symbol locks, same-symbol dice and eligible modifiers stay green, everything else dims. Add more dice of the same symbol. Commit label reflects the symbol: "Deal X melee damage", "Gain X shields", "Gain X resources", "Disrupt X resources", "Discard X cards", "Focus X dice". For damage: opponent characters highlight red, must select a target per die. For indirect damage: commit without targeting — opponent then distributes. For shields: own characters highlight blue, must select a target. Modifier dice cannot be tapped first; they light up only after a non-modifier of the same symbol is selected.
  - **Focus:** select focus dice → choose new faces for up to X dice in pool (face picker shown per die) → Commit = "End focus". Focus chains if a newly chosen face is also focus.
  - **Claim battlefield:** tap the battlefield card in the avatar bar (when green) → Commit = "Claim". No confirmation. After claim, player auto-passes all remaining turns this round.
  - **Card ability badges:** circular badge on in-play cards with Action/Power Action abilities. Green when usable. Tap → Commit = "Use ability". Power action badges grey out after use for the round.
  - **Discard to reroll:** left slot of action bar (when idle, hand not empty, dice in pool). Step 1 — hand expands to card-chooser, pick a card. Step 2 — Commit = "Discard card". Step 3 — dice highlight green, pick any number. Step 4 — Commit = "Reroll". Undo walks back one step at a time.
  - **Extra actions:** granted immediately after the triggering action resolves (Ambush, etc.). Player must take or decline before anything else. The board returns to the idle green highlight state for the extra action.
- **Live Preview of opponent actions.** While the active player is mid-flow (activating, resolving, face-picking, rerolling), their `ActiveFlow` is broadcast to the opponent via a fire-and-forget `game.preview` Socket.io event. The opponent's client renders this as visual feedback in real time — card tilt on activation, dice highlights during resolve, face-pick overlays, reroll targets — without waiting for the committed `game.state`. Dice tumble on the opponent side is driven by the authoritative `game.events` event (which carries the actual rolled faces) rather than the preview, so what the opponent sees during tumble is always server-verified.
- **3D dice architecture.** Dice are a first-class physical element — they must look and feel like real dice, not flat UI tiles.
  - **Board layer (Three.js):** A persistent Three.js canvas embedded in the dice pool areas. Each die is a `RoundedBoxGeometry` (chamfered edges matching physical dice) with proper lighting and the correct face showing up. At rest they look like dice sitting on a table viewed from a slight overhead-left angle. When a character enters the activate flow (pre-roll), that character's dice begin the pre-roll tumble animation on the board.
  - **Face picker (focus + events):** when a die face must be chosen, a compact panel shows all 6 faces of that die. Tapping a face smoothly rotates the 3D die (quaternion slerp) to show the chosen face on top. The panel is always the primary UX; direct die rotation is a secondary power-user path.
  - **Roll cam (backlog):** A full-screen physics roll animation on commit Roll Dice was prototyped and removed — the feel wasn't right. The plan when revisiting: same Three.js stack as the board layer, six individually-textured faces per die, physics-based settle, then a hard cut back to the board which already shows the server-authoritative result. See backlog.
  - **Why Three.js:** CSS 3D cannot produce chamfered cube edges or proper lighting. Three.js gives real geometry for the board layer and keeps the persistent rendering lightweight.
...
- **Phone live-match layout.** The board layout described above is the phone layout. Desktop inherits it — there is no separate desktop design. One layout, one codebase, designed for 360 pt portrait first. The design target is a 360 × 640 phone held in portrait; anything wider gets more breathing room but no structural changes.

> **Working agreement #10** below codifies this: every interaction must have a touch path. Reviewers reject changes that break it.
>
> **Exception — card designer (`apps/web/src/routes/designer/`).** The designer is a desktop-only internal authoring tool. Touch paths, 44 × 44 tap targets, and mobile breakpoints are not required there. Hover affordances, compact controls, and desktop-only layouts are fine in designer code.

---

## Game Engine Design Principles

1. **Server-authoritative. Zero trust on the client.** Clients render state; they never own it. Every action is a message to the server; the engine validates and applies it. Game state — whose turn it is, how much health a character has, whether a power action has been used this round — lives exclusively in `GameState` returned by the engine. The client never tracks game state locally (not in Zustand, not in component state, not in localStorage) because a client can lie. If the engine doesn't enforce a rule, the rule isn't enforced.
...
- **Step model.** Abilities carry `steps: EffectStep[]` (not a flat `effects` array). Each `EffectStep` is `{ effects: Effect[], then?: boolean }`. Effects within a step are AND-grouped: the step is fully resolved iff every effect's `fullyResolved` flag is true. `applySteps` in `dispatch.ts` iterates steps sequentially. Per-op `fullyResolved`: `gainResources` — always true; `drawCards`/`loseResources` — full amount only; `dealDamage`/`addShields`/`removeShields`/`healDamage` — ≥1 event with `amount > 0` fired; die ops (`removeDie`/`turnDie`/`modifyDieValue`) — requested count matched.
- **"Then" gating.** A step with `then: true` only executes when the previous step was fully resolved (AND of all its effects). If the previous step was not fully resolved, the then-step and all subsequent then-chained steps are skipped entirely. Target slots for skipped steps are not consumed.
...
10. **Touch-first input, always — except the card designer.** Every player-facing interaction must have a touch path: no hover-only affordances, no right-click-only menus, no precision-drag-required actions, tap targets ≥ 44 × 44 CSS pixels. Desktop is a touch UI with a cursor on top — not a separate mouse-and-keyboard design that we'll port to mobile later. See [Input model](#input-model). Reviewers reject changes that break this. **Exception:** `apps/web/src/routes/designer/` is a desktop-only internal authoring tool and is exempt from touch requirements.
...
14. **Follow the [Google Developer's Style Guide](https://developers.google.com/style) for all writing.** Documentation, UI copy, and player-facing text uses sentence case (first word + proper nouns only), active voice, and plain language. This applies to README, CLAUDE.md, UI labels, button text, error messages, and tooltip copy.
