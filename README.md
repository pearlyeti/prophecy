...
- **3D dice architecture.** Dice are a first-class physical element — they must look and feel like real dice, not flat UI tiles. Two rendering layers, clean boundary between them:
  ...
  - **Dice Results Cam (dice-box overlay):** When the player commits Roll Dice, a full-screen physics overlay fires. `@3d-dice/dice-box` (Babylon.js + Ammo.js WASM) runs real physics — dice tumble, bounce off each other, and settle naturally. Server-determined result values are passed in so the physics guides dice to land on the correct faces. This is the "sports broadcast camera cut" — the game board cuts away to the roll camera, results play out, then cuts back. dice-box is lazy-loaded at game-start (background fetch) so the WASM payload (~2MB) is ready before the first roll.
  - **Cut back:** when the player dismisses the Results Cam (tap/swipe), the overlay sweeps away. The board Three.js dice already show the correct faces (game state updated while the cam was playing). No fly-to-position animation needed — the cut handles the transition.
  - **Face picker (focus + events):** when a die face must be chosen, a compact panel anchored to the die shows all 6 faces. Tapping a face spins the 3D die to that face and confirms. Direct manipulation (dragging the 3D die to rotate) is a secondary power-user path. The panel is always the primary UX because players need to see all options simultaneously to make a strategic choice.
  - **Why these tools:** CSS 3D cannot produce chamfered cube edges or proper lighting — it will always feel fake to players who handle physical dice. Three.js gives real geometry for the board layer. dice-box's overlay/clear model is exactly right for the Results Cam and its physics quality justifies the bundle cost for a game where dice rolls are the central interaction. Three.js direct (not dice-box) for the board layer keeps the persistent rendering lightweight.
