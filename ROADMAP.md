...
## Done
- **2026-05-17 — ENGINE-CH1 — `choose` op (modal "or" / "Choose N of M").** `ChooseEffect` type added to `abilities/types.ts` (recursive type: branches contain `EffectStep[]`). `PendingChoice` interface and `pendingChoice: PendingChoice | null` added to `GameState` in `state/types.ts`; initialised `null` in `newGame()`. `applyChoose` in `dispatch.ts` sets `pendingChoice` and suspends (same pattern as `searchDeck`). `applySteps` extended with a second suspension block for `pendingChoice`. New `resolve-choice.ts` action validates picks (count, in-range, distinct), runs chosen branches via `applySteps` in order, clears `pendingChoice`, resumes `remainingSteps`. `getLegalActions` blocks all actions while `pendingChoice` is set; exposes `canResolveChoice`. `choose` Zod schema (with `z.lazy()` for recursive `steps`) replaces `stub('choice')` in `effectSchema`; added to `KNOWN_OPS`. AbilityBuilder `choose` form: count input, branch list with add/remove, nested `StepsList` per branch; "Choose…" entry in implemented-ops optgroup. 16 new tests; 285 engine tests green; workspace typecheck clean. (`db41815`)
- **2026-05-17 — DESIGNER-HL1 — Humanize designer labels + costs helper text.** Created `apps/web/src/routes/designer/labels.ts` with `humanize()` (camelCase → sentence case, Google Developer's Style Guide) and `COST_HELPER` per-cost description map. Applied `humanize()` throughout `AbilityBuilder.tsx`: ability-kind header and "Add ability" buttons, play condition dropdown, trigger event optgroups, action cost dropdown, effect op optgroups, and `TargetPicker` options. Added italic helper text per action cost row. Updated `README.md` (working agreement #14) and `CLAUDE.md` (`## Style`) to document the Google Developer's Style Guide as the project writing standard. Typecheck clean; JSON round-trip unaffected.
- **2026-05-17 — ENGINE-ST1 — Step model + `then` gating.** Replaced flat `effects: Effect[]` on all ability kinds with `steps: EffectStep[]` (`EffectStep = { effects: Effect[]; then?: boolean }`). `applySteps` in `dispatch.ts` iterates steps with then-gating: a step with `then: true` is skipped if the previous step did not fully resolve (AND of all effect `fullyResolved` flags). Per-op `fullyResolved` added to every handler: `gainResources` always true; `drawCards`/`loseResources` only if full amount; `dealDamage`/`addShields`/`removeShields`/`healDamage` only if ≥1 shield/damage event with `amount > 0` fired; die ops by count matched. `PendingSearch.remainingSteps` replaces `remainingEffects`; `QueueEntry.steps` replaces `QueueEntry.effects`; all 9 action-file callers updated. 25 seed card JSONs migrated from flat `effects: [...]` to `steps: [{effects:[...]}]`. New then-gating tests: (a) removeShields→dealDamage gates correctly; (b) multi-effect AND step gates the then-step; (c) per-op fullyResolved for gainResources, drawCards (empty deck). `AbilityBuilder.tsx` replaced `EffectsList` with `StepsList`: "Then ↳" toggle per step, "+ Add effect to step" inside each step, "+ Add step" below the list. 269 engine tests green; workspace typecheck clean. (`e11c1d7`)
...
- **2026-05-17 — DESIGNER-TYP — Normalize designer typography.** Replaced all `text-[10px]`/`text-[11px]` arbitrary sizes with `text-xs` (12 px) across 8 designer files; replaced all `text-neutral-*` text content colors with `text-foreground` / `text-muted-foreground` semantic tokens; unified form controls to `h-9 text-sm`. Typecheck clean. (`fcb3a86`)
