...
## Done
- **2026-05-17 — SERVER-3 — Session-gate designer write routes.** Added `requireSession(req, res): Promise<string | null>` helper to `apps/game-server/src/index.ts`: forwards incoming cookies to `{API_URL}/api/auth/get-session`, returns `userId` on success, writes `401` and returns `null` on failure. Called at the top of all 6 mutating designer handlers (`PUT /designer/cards`, `PUT /designer/decks`, `PUT /designer/attributes`, `PUT /designer/card-art/:cardId`, `POST /designer/commit`, `POST /designer/ai/parse-abilities`) before the existing `DESIGNER_SECRET` guard (which stays as belt-and-suspenders for local dev). GET routes remain open. Typecheck clean.
