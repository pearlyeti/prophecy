...
## Done
- **2026-05-17 — ENGINE-PC1 — Enforce `playCondition` in the engine.** Added `playConditionMet(state, playerId, condition)` to `state/legal-actions.ts` covering all 8 existing condition kinds (`controlsBattlefield`, `spotCharacter`, `spotCard`, `moreReadyCharacters`, `firstActionOfRound`, `opponentHasNoCards`, `haveNCharactersInPlay`, `opponentHasNCharacters`). Added `actionsThisRound: Record<string, number>` to `GameState` (reset each upkeep) to implement `firstActionOfRound`. Wired into `getLegalActions` (excludes abilities from `actionableCardIds` / `powerActionableCardIds` when unmet) and into the `use-card-action` pre-dispatch path (throws `IllegalActionError` with `playCondition` in the reason). Same gate added to `play-card` immediate abilities, `activate` before-triggers, `collectAfterTriggers`, and `collectBeforeTriggers`. 23 new tests (16 `playConditionMet` unit tests, 2 legal-actions gating tests, 4 use-card-action pre-dispatch tests + 1 existing enhanced). All 292 engine tests pass; monorepo typecheck clean. (`48208bd`)
