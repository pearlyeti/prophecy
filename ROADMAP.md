...
## Done
- **2026-05-15 — API-1 — Apply first DB migration against real Postgres.** Migration `0000_deep_stellaris.sql` applied cleanly against Postgres 16. All 7 tables and 5 enums confirmed via psql. Fixed `migrate.ts` to use the same local-dev default as `drizzle.config.ts` (no longer errors when `DATABASE_URL` is unset). Fixed README `docker compose up -d` → `docker compose -f infra/docker-compose.yml up -d`. Typecheck clean.
