...
## Done
- **2026-05-17 — WEB-24 — Character-target commit/preview for card plays.** `pendingCharTargetPlay` added to `ActiveFlow` union (store.ts). `hasImmediateRevealOrRoll` helper gates non-undoable cards (searchDeck, rollEventDie, rollCardDie) to immediate dispatch; all others enter the preview state. Drag-release over a valid character target sets `pendingCharTargetPlay` instead of dispatching. Amber preview badge on target character shows net effect: `−N` for dealDamage (shields-subtracted), `+N shields`, `−N shields`, `+N HP`. Amber ring via `RING.pendingPreview`. Commit button: amber, labelled "Play [card name]". Undo clears flow; card stays in hand. `docs/rules-reference.md` gains "Undoable vs. committed actions" subsection. Typecheck clean. (hash TBD)
