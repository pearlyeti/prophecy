...
6a. **Consider an E2E test.** For any card that ships a user-visible flow end-to-end (matchmaking, a new game action, reconnect, a storefront purchase, etc.), ask: does a Playwright test belong here? If yes, add it to `apps/web/e2e/` in the same PR — see `apps/web/README.md` § E2E tests for the pattern. If the flow is purely internal or already covered, note why in the PR description. Don't leave this as a follow-up.
