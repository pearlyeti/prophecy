...
export interface LegalActions {
  ...
  readonly canChooseBattlefield: boolean;
...
export function getLegalActions(state: GameState, playerId: string): LegalActions {
  ...
  if (state.phase === 'setup' && state.setup) {
    ...
    return {
      ...EMPTY,
      canConcede: true,
      canChooseBattlefield: isWinner && state.setup.step === 'choose-battlefield',
      canPlaceShield:
  ...
  return {
    ...
    canConcede: true,
    canReroll: isMyTurn && player.handCount > 0 && player.diceInPool.length > 0,
    activatableCharacterIds: activatable,
    resolvableSymbols: [...resolvable],
    canPlayCard: isMyTurn && player.handCount > 0,
    canChooseBattlefield: false,
