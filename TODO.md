...
### Done
- **2026-05-15 — WEB-3D-2 — Dice Results Cam.** (`8edc60b`) `ResultsCam.tsx` lazy-loaded; `@3d-dice/dice-box` (Babylon.js + Ammo.js WASM physics) runs a full-screen physics roll overlay when the player commits Roll Dice. Dispatch to server and show cam fire simultaneously. `onRollComplete` callback reveals "Tap to continue"; tap or swipe-up triggers a sweep-out transition then unmounts. Board Three.js dice already show correct server-authoritative faces on cut-back. WASM bundle preloaded in background at game-start. Dice-box assets copied to `public/assets/dice-box/`. Typecheck clean.
